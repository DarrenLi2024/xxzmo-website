...
interface SearchClientProps {
  initialQuery?: string;
  initialTag?: string;
  initialResults?: ArticleResult[] | null;
}
...
export function SearchClient({
  ...
  const [results, setResults] = useState<ArticleResult[]>(initialResults || []);
  const [loading, setLoading] = useState(false);
  const [searched, setSearched] = useState(Boolean(initialQuery || initialTag));
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (query === initialQuery && activeTag === initialTag && initialResults) {
        return;
      }
      doSearch(query, activeTag);
    }, 300);
    return () => clearTimeout(timer);
  }, [query, activeTag, doSearch, initialQuery, initialTag, initialResults]);
        ...
        <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-lg">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-amber-800 font-medium">标签筛选：</span>
              <span className="text-sm text-amber-900 font-bold">{activeTag}</span>
              <span className="text-xs text-amber-600">（{results.length} 篇相关诗文）</span>
            </div>
            <button
              onClick={clearTag}
